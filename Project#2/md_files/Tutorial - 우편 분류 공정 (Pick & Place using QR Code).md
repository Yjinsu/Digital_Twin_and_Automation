...
카메라를 계속 키고 있을 경우, 프레임이 밀리는 현상이 발생함에 따라 로봇이 제대로 동작하지 않았습니다.
따라서 Main문을 **'qr_classify_robot.py'** 와  **'qr_code_reader.py'** 두 코드로 쪼개어 설계했습니다.
로봇이 일련의 우편 분류 동작을 완료하고 홈위치에 도착했을 경우에만 카메라를 동작시키고, 이미지를 인식한 이후에는 다시금 카메라를 종료하는 식으로 **'qr_code_reader.py(카메라 인식 관련 코드)'** 를 작성했는데,
이 경우 **'qr_classify_robot.py(로봇을 동작하는 main문)'** 안에서 카메라 이미지를 지속적으로 출력할 수 없다는 문제점이 있었습니다.
결국 카메라 두 개를 활용해서 영상을 촬영하고, 편집을 통해 데모 영상을 완성했습니다.
